...
Steps:
1. Call get_comprehensive_context to see current state BEFORE liking
2. Call like_post with the identifier
3. Call get_comprehensive_context to verify the like was successful AFTER liking
4. Check if like button changed state (filled heart vs outline)
...
- If post doesn't show as liked in the after-screenshot, report failure""",
            "tools": [tool_dict["like_post"], tool_dict["get_comprehensive_context"]]
        },
...
Steps:
1. Call get_comprehensive_context to see current state BEFORE commenting
2. Call comment_on_post with post identifier and comment text
3. Call get_comprehensive_context to verify the comment appeared AFTER commenting
4. Check if your comment is visible in the replies
5. Return success/failure based on visual verification

CRITICAL:
- Do NOT comment on multiple posts.
- Do NOT trust tool feedback alone - verify with screenshots
- If comment doesn't appear in the after-screenshot, report failure

NOTE: The comment text should already be in the user's writing style (generated by the main agent).""",
            "tools": [tool_dict["comment_on_post"], tool_dict["get_comprehensive_context"]]
        },
