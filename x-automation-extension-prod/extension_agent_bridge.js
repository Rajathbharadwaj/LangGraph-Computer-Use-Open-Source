  ...
  try {
    ...
    switch (type) {
      ...
      case 'COMMENT_ON_POST':
        result = await commentOnPost(command.post_identifier, command.comment_text);
        break;
...
/**
 * Comment on a post - MORE RELIABLE than Playwright version
 * Uses article context and data-testid selectors
 */
async function commentOnPost(postIdentifier, commentText) {
  console.log(`💬 Commenting on post matching: "${postIdentifier}"`);
  console.log(`💬 Comment text: "${commentText}"`);

  try {
    // Step 1: Find the post using article context
    const post = findPostByIdentifier(postIdentifier);
    if (!post) {
      throw new Error(`Post not found matching: "${postIdentifier}"`);
    }

    console.log('✅ Post found in timeline');

    // Step 2: Find reply button WITHIN the same article
    const replyButton = post.querySelector('[data-testid="reply"]');
    if (!replyButton) {
      throw new Error('Reply button not found in post');
    }
    ...
    replyButton.click();
    await sleep(1500); // Wait for dialog to open

    // Step 4: Wait for reply dialog to appear
    ...
    // Step 5: Find textarea INSIDE the dialog
    const textarea = dialog.querySelector('[data-testid="tweetTextarea_0"]');
    if (!textarea) {
      throw new Error('Reply textarea not found in dialog');
    }
    ...
    // Use proper React event triggering
    ...
    // Step 7: Find submit button INSIDE the dialog
    const submitButton = dialog.querySelector('[data-testid="tweetButton"]');
    if (!submitButton) {
    ...
    if (submitButton.disabled || submitButton.getAttribute('aria-disabled') === 'true') {
      throw new Error('Submit button is disabled - text may not have been entered correctly');
    }
    ...
    // Step 9: Verify dialog closed (indicates success)
    const dialogStillExists = document.querySelector('[role="dialog"]');
    if (dialogStillExists) {
      console.log('⚠️ Dialog still open - comment may have failed');
    } else {
      console.log('✅ Dialog closed - comment likely posted');
    }

    // Step 10: Try to verify comment appears in the thread
    await sleep(1000);
    const allPosts = document.querySelectorAll('article[data-testid="tweet"]');
    let commentFound = false;

    for (const p of allPosts) {
      const text = p.innerText.toLowerCase();
      if (text.includes(commentText.toLowerCase())) {
        commentFound = true;
        console.log('✅ Comment verified in thread');
        break;
      }
    }
